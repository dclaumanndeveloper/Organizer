...
17. **Perfis Nomeados** (opcional): além do "último uso" lembrado automaticamente, é possível salvar várias combinações de pasta + opções sob um nome (ex: "Downloads", "Fotos do trabalho") no campo "Perfil (configurações salvas)". Digite um nome e clique em "Salvar perfil" para gravar a pasta, o ano mínimo e todas as opções marcadas; selecione um perfil salvo na lista para recarregar tudo de uma vez; "Excluir perfil" remove o perfil selecionado. Os perfis ficam salvos em `~/.organizador_arquivos/perfis.json`.
